...
class LatentDiffusionModel(nn.Module):
    def __init__(self, latent_dim, hidden_dims, max_freq, num_bands, std=0.2):
        ...
        # 最后一层输出与原始潜在维度相同
        self.layers.append(nn.Linear(input_dim, latent_dim[0] * latent_dim[1]))
    ...
    def forward(self, x, t):
        ...
        return x.view(batch_size, *self.latent_dim)
...
#         for hidden_dim in hidden_dims:
#             self.layers.append(nn.Linear(input_dim, hidden_dim))
#             self.layers.append(nn.BatchNorm1d(hidden_dim))  # 添加BatchNorm层
#             self.layers.append(nn.ReLU(inplace=True))
#             input_dim = hidden_dim  # Set input dim for next layer
...
#                 x = layer(x)
#                 if i < len(self.layers) - 1:  # 不在输出层应用BatchNorm
#                     x = self.layers[i+1](x)  # 应用BatchNorm
#                     x = self.layers[i+2](x)  # 应用ReLU
#             elif isinstance(layer, nn.BatchNorm1d) or isinstance(layer, nn.ReLU):
#                 continue  # 已在上面的if语句中应用这些层
#             else:
#                 raise NotImplementedError("Unknown layer type found in layers")
...
def train(model, model_ema, autoencoder, opt, rng, train_dl,scaler, epoch, ema_decay, device):
...
def save(model, model_ema, opt, scaler, epoch):
    ...
    #     os.mkdir('./ckpts')
    filename = '/home/ziran/se3/EFEM/lib_shape_prior/dev_ckpt/latent_diffusion_3l.pth'
    obj = {
...
class CustomDataset(Dataset):
    def __init__(self, codebook):
        feat = torch.cat([codebook['z_so3'], 
                  codebook['z_inv'].unsqueeze(2), 
                  codebook['scale'].unsqueeze(1).unsqueeze(1).repeat(1,256,1), 
                  codebook['center'].repeat(1,256,1)], dim=2)
...
def main():
    ...
    opt = optim.Adam(model.parameters(), lr=2e-4,weight_decay=1e-6)
    ...
    while True:
        print('Epoch', epoch)
        train(model, model_ema, autoencoder, opt, rng, train_dl, scaler, epoch, ema_decay, device)
        epoch += 1
        # if epoch % 10 == 0:
        #     val(model_ema, autoencoder, val_dl, device, seed, epoch)
        #     demo(model_ema, autoencoder, val_dl, device, seed, epoch, steps, eta)
