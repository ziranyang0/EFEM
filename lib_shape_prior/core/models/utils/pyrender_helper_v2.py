...
#     del renderer
# except:
#     logging.warning("PYRENDER EGL BACKEND FAIL, TURN TO OSMESA!")
#     from importlib import reload
#     os.environ["PYOPENGL_PLATFORM"] = "osmesa"
#     pyrender = reload(pyrender)
...
def render(
    ...
    **kargs,
    ...
    cam_pose = np.eye(4)
    R = euler2mat(cam_angle_yaw, -cam_angle_pitch, cam_angle_z, "ryxz")
    cam_pose[:3, :3] = R
    cam_pose[2, 3] += cam_dist
    cam_pose[:3, 3:] = R @ cam_pose[:3, 3:]
    cam_pose[1, 3] += cam_height
    ...
    if pcl_list is not None:
        ...
        for i in range(len(pcl_list)):
            ...
            if radius <= 0.0:
                ...
                sm = trimesh.creation.uv_sphere(radius=radius, count=UVSPH_COUNT)
    ...
    # rgb, _ = renderer.render(scene, flags=pyrender.constants.RenderFlags.RGBA)
